...
async def metrics(target: str):
    try:
        ...
        LAN_IPV4_AND_IPV6_STATUS.set(1 if result.data.lan.ipv4_and_ipv6 == True else 0)
        LAN_IP_ADDRESS.info({
                "ipv4": result.data.lan.ipv4,
                "ipv6": result.data.lan.ipv6 if result.data.lan.ipv6 != None else "N/A"
        })
        WLAN_IPV4_AND_IPV6_STATUS.set(1 if result.data.wlan.ipv4_and_ipv6 == True else 0)
